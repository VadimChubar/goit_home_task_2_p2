...
# сиворити клас який буде управляти всім ботом, поки що відкласти в сторону команди користувача.
# лише абстрактний клас який буде містити необхідні методи для маніпулювання даними
...
##################################################################
    ...
    def birthday_in_days(self, number_days):
        ...
        checked_date = datetime(year=datetime.now().year, month=datetime.now(
        ).month, day=datetime.now().day + int(number_days))
        flag_birth = 0
        for key, value in add_book.items():
            record_date = datetime(year=datetime.now(
            ).year, month=add_book.data[key].record_dict['Birthday'].month, day=add_book.data[key].record_dict['Birthday'].day)
    ...
    if len(add_book.notes_data) == 0:
        ...
        for key, value in add_book.notes_data.items():
            if key.lower().find(text.lower()) >= 0:
                print(
                    f'Looked up text was found in tag :"{key}" for next Note: "{value}"')
                flag_found += 1
            if value.lower().find(text.lower()) >= 0:
                print(
                    f'Looked up text was found in Notes :"{value}" with next tags: "{key}"')
                flag_found += 1
...
def on_press(key):
    ...
    try:
        if key == keyboard.Key.esc:
            ...
            print(
                '                                                                                    ', end='\r')
            print(''.join(map(str, main_list_guess)),
                  end='\r\t\t\t\t', flush=True)
